...
public class Utils {
    ...
    private static final SimpleDateFormat ISO_DATETIME_FORMAT = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.ENGLISH);
    private static final SimpleDateFormat ISO_DATE_FORMAT = new SimpleDateFormat("yyyy/MM/dd", Locale.ENGLISH);
    private static final SimpleDateFormat ISO_DATE_FORMAT_2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH);
    ...
    public static void showErrorAndFinish(final Activity activity, Exception ex) {
        try {
            ...
            new AlertDialog.Builder(activity)
//                    .setMessage(Utils.getErrorMessage(ex))
                    .setMessage(activity.getString(R.string.error_in_the_scan))
                    .setCancelable(false)
...
//                            activity.finish();
    ...
    */
    ...
    public static String isoDateTimeFormat2(GregorianCalendar calendar) {
        return ISO_DATE_FORMAT_2.format(calendar.getTime());
    }
    ...
    public static Spanned longDateFormat(Calendar date) {
        ...
        String s = DateFormat.getLongDateFormat(MainApplication.getInstance()).format(date.getTime());
    ...
    public static Spanned timeFormat(Calendar date) {
        if (date == null) return new SpannableString("");
        String s = DateFormat.getTimeFormat(MainApplication.getInstance()).format(date.getTime());
